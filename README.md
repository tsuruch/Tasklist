...
- 名前の規則性に統一感がない箇所がある
  - 手当たり次第開発してその都度必要なTableやColumnを追加していったため、命名規則に統一感がない箇所がちらほらある。ゆえにその名前に応じたコードを書かねばならず、コードがやや冗長化した箇所がある
  - 何が良くないか：今後アプリ開発においてより複雑化してくると拡張性や作業量、不具合の原因になりうる。
  - 改善すべきこと：命名規則についての基礎的な知識。また、アプリ開発する際にはどのTableが必要でどのColumnが必要が前もって吟味する必要があり、命名規則についてのメモや共有を残しとくのがいいのかもしれない。
- 外部キー制約を付けるかつけざるか
  - とりあえず大体リレーションを組んでいるが、コメントTableなどには就けないほうがいい気がするなど、外部キーを何のためにつけるのか判断があいまい。
  - 例えば：コメント投稿者（親）が削除されるとコメント（子）も消されるのはまずい。なぜならコメントなどは、だれが投稿したかどうかの前にそのコメント自体に価値があるため。
  - 改善すべきこと：
